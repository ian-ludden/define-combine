...
"""
Convert an index between 0 and num_cols * num_rows - 1 
to a (row, col) tuple. 
"""
def index_to_row_col_tuple(index, num_cols):
    return(index // num_cols, index % num_cols)


"""
Build a Gurobi MIP model for the (mixed-integer) high-point relaxation. 
"""
def build_model(num_rows, num_cols, num_districts, unit_ids, unit_weights):
    ...
    try:
        ...
        x_edges = set()
        for u in unit_ids:
            row_u, col_u = index_to_row_col_tuple(u, num_cols)
            for v in unit_ids:
                row_v, col_v = index_to_row_col_tuple(v, num_cols)
...
def build_model_continuous(num_rows, num_cols, num_districts, unit_ids, unit_weights):
    num_units = num_rows * num_cols
    num_subdistricts = 2 * num_districts
    units_per_district = num_units // num_districts
    units_per_subdistrict = num_units // num_subdistricts
    ...
    try:
        ...
        # Create variables x_{ij}, indicators for whether 
        # unit i is assigned to (the subdistrict centered at) unit j
        x = m.addVars(unit_ids, unit_ids, name="x", vtype=GRB.CONTINUOUS)

        # Create variables y_{ij}, indicators for whether 
        # unit i is assigned to (the district centered at) unit j 
        y = m.addVars(unit_ids, unit_ids, name="y", vtype=GRB.CONTINUOUS)

        # Create variables a_{j} and b_{j}, indicators for whether 
        # the district centered at j is a win for party a (resp., party b)
        a = m.addVars(unit_ids, name="a", vtype=GRB.CONTINUOUS, obj=-1)
        b = m.addVars(unit_ids, name="b", vtype=GRB.CONTINUOUS, obj=1)
        ...
        # Create cut-edges variables
        c = m.addVars(all_directed_edges, unit_ids, unit_ids, name="c", vtype=GRB.CONTINUOUS)

        # Create both-units-are-subdistrict-centers variables
        z = m.addVars(unit_ids, unit_ids, name="z", vtype=GRB.CONTINUOUS)
...
"""
Print summary of subdistrict plan and district plan 
produced by solving the given MIP model. 
To be called after `m.optimize()`. 
"""
def summarize_model_results(m, unit_ids, unit_weights, num_cols):
    ...
    x_vals = np.zeros((len(unit_ids), len(unit_ids)))
    ...
    y_vals = np.zeros((len(unit_ids), len(unit_ids)))
    ...
    print('Ties:      \t', len(y_partition_dict) - count_a_wins - count_b_wins)
